...
class MainFormBKP extends React.Component {
  state = {
    isLoading: true,
    prodLocQty: {},
  ...
  generateTotals = () => {
    ...
    console.log(prodLocQty);

    Object.keys(prodLocQty).map((productId) => {
      const price = products[productId].price;
      Object.keys(prodLocQty[productId]).map((locationId) => {
        const total = prodLocQty[productId][locationId]["qty"] * price;
        if (pTotals[productId]) pTotals[productId] += total;
        else pTotals[productId] = total;

        if (lTotals[locationId]) lTotals[locationId] += total;
        else lTotals[locationId] = total;
  ...
  renderProductTotals = () => {
    ...
    return Object.values(products).map((product) => {
      const total = productsTotal[product.entity_id]
        ? productsTotal[product.entity_id]
        : 0;
  ...
  renderLocationTotals = () => {
    const products = getProducts();
    return Object.values(products).map((product) => {
      const total = this.getProductTotal(product.entity_id);
      return (
        <div
          className="col col-31 border-right total-column"
          data-lid="207"
          key={`ltotal${product.entity_id}`}
        >
  ...
  setQty = (productId, locationId, qty) => {
    let prodLocQty = this.state.prodLocQty;
  ...
  testClick = (e) => {
    ...
    // let prodLocQty = this.state.prodLocQty;
    // prodLocQty[6000][209] = { qty: 25 };
    // this.setState({ prodLocQty: prodLocQty });
  };

  componentDidMount = () => {
    this.setState({ prodLocQty: getProdLocQty() });
    this.generateTotals();
